...
export const ConnectMercadoPago = ({
  ...
  const mutation = trpc.mercadoPago.disconnect.useMutation({
    onSuccess: () => {
  ...
  const handleDisconnect = async () => {
    const confirmed = await confirm(
      "Tem certeza?",
      "Você está prestes a desconectar sua conta do Mercado Pago. Isso pode afetar suas vendas."
    );
    ...
    return (
      <Card className="w-full md:w-1/2">
        <CardHeader>
          ...
          <CardDescription>
            Para que você possa receber os pagamentos diretamente em sua conta,
            é necessário conectá-la ao nosso sistema. Isso garante que o valor
            das suas vendas seja repassado automaticamente para você de forma
            segura, rápida e transparente. Ao conectar, você autoriza a
            plataforma a criar cobranças em seu nome e a repassar uma pequena
            taxa de intermediação. Esse processo é realizado pelo próprio
            Mercado Pago e você pode desconectar sua conta a qualquer momento.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Link
            ...
            href={`https://auth.mercadopago.com.br/authorization?client_id=${process.env.NEXT_PUBLIC_MP_CLIENT_ID}&response_type=code&platform_id=mp&redirect_uri=${process.env.NEXT_PUBLIC_MP_REDIRECT_URI}`}>
            <Button className="w-64 bg-blue-700 hover:bg-blue-500">
              <svg
                role="img"
                viewBox="0 0 24 24"
  ...
  return (
    <Card className="w-full md:w-1/2">
      <CardHeader>
        <CardTitle className="text-green-500">Conta conectada</CardTitle>
        <CardDescription>
          Sua conta Mercado Pago já está conectada. Você pode desconectar
          cliando no botão abaixo
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Button
